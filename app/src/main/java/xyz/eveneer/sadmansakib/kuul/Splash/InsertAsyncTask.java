...
class InsertAsyncTask extends AsyncTask<PhoneNumber,Void,Void> {
    PhoneNumberDao mPhoneDao;

    InsertAsyncTask(PhoneNumberDao phoneDao) {
        mPhoneDao = phoneDao;
    }

    @Override
    protected Void doInBackground(PhoneNumber... phoneNumbers) {
        mPhoneDao.insert(phoneNumbers[0]);
        return null;
    }
}
